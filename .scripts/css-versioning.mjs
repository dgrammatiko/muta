...
const opts = {
  version: (imagePath, sourceCssPath) => {
    ...
    if (
      ...
      existsSync(`${dirname(sourceCssPath)}/${imagePath}`)
    ) {
      const hashSum = crypto.createHash('md5');
      hashSum.update(
        readFileSync(resolve(`${dirname(sourceCssPath)}/${imagePath}`)),
      );
...
function replaceFontVer(css) {
  const vers = {};
  const matches = css.matchAll(/url\(\".+fa-(?<name>.+)\.woff2\?v=(?<ver>.+)\"\)\sformat\(\"woff2\"\)/g);
  ...
  const fileContent = `<?php /** This file is automatically generated */ defined('_JEXEC') || die; return [${Object.keys(vers)
    .map((key) => `'${key}' => '${vers[key]}'`)
    .join(', ')}];`;

  writeFileSync(
    'src/templates/administrator/muta/src/Helper/versions.php',
    fileContent,
...
const fixVersion = async (file) => {
  try {
    ...
    const data = await Postcss([UrlVersion(opts)]).process(cssString, {
      from: file,
    });
    await writeFile(file, data.css, { encoding: 'utf8', mode: 0o644 });

    console.log(`Versioning ${file}...`);
    if (file === 'media/templates/administrator/muta/css/vendor/fontawesome-free/fontawesome.css') {
...
cssVersioning()
  .then(() => getFiles('./media/templates/administrator/muta/'))
  .then(async (files) => {
